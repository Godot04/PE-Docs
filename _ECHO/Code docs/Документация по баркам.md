...
**Зачем нужно:** Голосовая маска может менять не только имя, но и барк. Это событие происходит через инвентарь (благодаря `IInventoryRelayEvent`), и голосовая маска берет его и подменяет данные.
...
Каждый обработчик вызывает соответствующий `SetBark*()` метод (определён в [HumanoidProfileEditor.Barks.cs](Content.Client/_ECHO/Barks/UI/HumanoidProfileEditor.Barks.cs)), а затем `UpdateSaveButton()` для активации кнопки сохранения.
...
#### Set-методы (в HumanoidProfileEditor.Barks.cs)
...
{
    Profile = Profile?.WithBarkMinVariation(Math.Clamp(variation,
        _cfgManager.GetCVar(UCCVars.BarksMinDelay),
        Profile!.Bark.MaxVar));
    ReloadPreview();
    SetDirty();
}
...
{
    Profile = Profile?.WithBarkMaxVariation(Math.Clamp(variation,
        Profile!.Bark.MinVar,
        _cfgManager.GetCVar(UCCVars.BarksMaxDelay)));
    ReloadPreview();
    SetDirty();
}
```
...
**Примечание:** После апстрима многие методы были вынесены в partial-файлы:
- `SetAge()`, `SetSex()`, `SetGender()`, `SetSpecies()`, `SetSpawnPriority()` → [HumanoidProfileEditor.Appearance.cs](Content.Client/Lobby/UI/HumanoidProfileEditor.Appearance.cs)
- `SetName()`, `UpdateNameEdit()` → [HumanoidProfileEditor.BasicInfo.cs](Content.Client/Lobby/UI/HumanoidProfileEditor.BasicInfo.cs)
- `UpdateSexControls()`, `UpdateSkinColor()`, `UpdateEyePickers()` и др. Update-методы → [HumanoidProfileEditor.Appearance.cs](Content.Client/Lobby/UI/HumanoidProfileEditor.Appearance.cs)
- Bark-методы (`SetBarkProto()`, `SetBarkPitch()` и т.д.) → [HumanoidProfileEditor.Barks.cs](Content.Client/_ECHO/Barks/UI/HumanoidProfileEditor.Barks.cs)
...
### 6. Content.Server/Database/ServerDbBase.cs

#### Запись в БД (метод ConvertProfiles):
```csharp
...
### 7. Content.Shared/Humanoid/HumanoidProfileSystem.cs
...
### 8. Content.Server/VoiceMask/VoiceMaskSystem.cs
...
### 9. Content.Shared/VoiceMask/VoiceMaskComponent.cs
...
Голосовая маска хранит свои настройки барка отдельно от персонажа. Также после апстрима добавлено новое поле:
```csharp
public bool ChangeIDName = false;  // Изменять ли имя на Agent ID
```

---

### 10. Content.Server/Speech/EntitySystems/VoiceOverrideSystem.cs
...
### 11. Content.Server/Speech/Components/VoiceOverrideComponent.cs
...
### 12. Content.Shared/VoiceMask/SharedVoiceMaskSystem.cs
...
1. Положите звуковой файл (.ogg) в `Resources/Audio/_ECHO/Barks/`. **Важно:** файл должен быть в формате **Mono** (одноканальный), а не Stereo. Это требование движка для позиционного аудио.
